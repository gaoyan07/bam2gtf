...
#define sg_update_edge_wei(ed, sj) { \
    ...
    ed.anc = (anc_t*)_err_malloc(sj->uniq_c * sizeof(anc_t)); \
    int _i; \
    for (_i = 0; _i < sj->uniq_c; ++_i) {   \
        ed.anc[_i].bid = sj->anc[_i].bid;   \
        ed.anc[_i].left_anc_len = sj->anc[_i].left_anc_len;   \
        ed.anc[_i].right_anc_len = sj->anc[_i].right_anc_len;   \
        ed.anc[_i].left_sj_len = sj->anc[_i].left_sj_len;   \
        ed.anc[_i].right_sj_len = sj->anc[_i].right_sj_len;   \
