...
{
    err_printf("[%s] constructing splice-graph with GTF file ...\n", __func__);
    ...
    gene_group_free(gg);
    err_printf("[%s] constructing splice-graph with GTF file done!\n", __func__);
    return sg_g;
}
...
void sg_dump_node(SGnode n, FILE *sg_fp)
{
    ...
    if (n.pre_n > 0) err_fwrite(n.pre_id, sizeof(uint32_t), n.pre_n, sg_fp);
    err_fwrite(&n.pre_domn_n, sizeof(int32_t), 1, sg_fp);
    if (n.pre_domn_n > 0) err_fwrite(n.pre_domn, sizeof(uint32_t), n.pre_domn_n, sg_fp);
    err_fwrite(&n.post_domn_n, sizeof(int32_t), 1, sg_fp);
    if (n.post_domn_n > 0) err_fwrite(n.post_domn, sizeof(uint32_t), n.post_domn_n, sg_fp);
}
...
void sg_restore_node(SGnode *n, FILE *sg_fp)
{
    ...
    if (n->pre_n > 0) err_fread_noeof(n->pre_id, sizeof(uint32_t), n->pre_n, sg_fp);
    ...
    if (n->pre_domn_n > 0) err_fread_noeof(n->pre_domn, sizeof(uint32_t), n->pre_domn_n, sg_fp);
    ...
    if (n->post_domn_n > 0) err_fread_noeof(n->post_domn, sizeof(uint32_t), n->post_domn_n, sg_fp);
}
...
void sg_dump_site(SGsite s, FILE *sg_fp)
{
    ...
    if (s.exon_n > 0) err_fwrite(s.exon_id, sizeof(uint32_t), s.exon_n, sg_fp);
}
...
void sg_restore_site(SGsite *s, FILE *sg_fp)
{
    ...
    if (s->exon_n > 0) err_fread_noeof(s->exon_id, sizeof(uint32_t), s->exon_n, sg_fp);
}
